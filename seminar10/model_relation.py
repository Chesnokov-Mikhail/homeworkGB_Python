...
'''
Получить запись новой или существующей связной таблицы для работы с ней
'''
def relation_get_record_person(id_person):
    if id_person:
        bd_record = common_function.bd_load(bd_path)
        if bd_record:
            try:
                for i in range(len(bd_record)):
                    # находим запись в связной таблице
                    if bd_record[i]['id_person'] == id_person:
                        return bd_record[i]
            except:
                print(f'Ошибка чтения данных записи с № {id_person} в связной БД {bd_path}')
                ...
                record_new = dict.fromkeys(model_record.keys())
                record_new['id_person'] = id_person
                return record_new
        else:
            print(f'Связная БД {bd_path} пуста')
            return False
